-- This migration will add new rpc functions for creating and updating product keys, using named fields in json objects rather than array indexes for better readability and maintainability.
-- Function to create a new product key
CREATE OR REPLACE FUNCTION create_product_key(
    new_product_data JSONB,
    new_recipe_data JSONB DEFAULT NULL
) RETURNS VOID
LANGUAGE plpgsql
AS $FUNCTION$
DECLARE
...
BEGIN
    ...
    IF new_recipe_data IS NOT NULL AND new_product.is_processed IS TRUE THEN
        ...
        INSERT INTO recipes (label, vendor_id) VALUES (
            recipe_label, new_product.vendor_id) RETURNING * INTO new_recipe;
...
-- Function to update an existing product key
CREATE OR REPLACE FUNCTION update_product_key(
    updated_product_data JSONB,
    updated_recipe_data JSONB DEFAULT NULL
) RETURNS VOID
LANGUAGE plpgsql
AS $FUNCTION$
DECLARE
...
BEGIN
    ...
    IF updated_recipe_data IS NOT NULL AND updated_product.is_processed IS TRUE THEN
        ...
        recipe_id_value := (SELECT recipe_id FROM recipe_elements WHERE product_id = updated_product.product_id AND type = 'output' LIMIT 1);
