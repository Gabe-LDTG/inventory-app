-- This migration adds a trigger to the `po_unit_allocations` table to log changes for auditing purposes. Also adds CRUD functions for the `po_unit_allocations` table.
-- Also updating the `get_purchase_orders_with_details` function to include po_raw_lines and invoices in the returned JSON.
CREATE TRIGGER tr_audit_po_unit_allocations
AFTER INSERT OR UPDATE OR DELETE ON public.po_unit_allocations
FOR EACH ROW
EXECUTE FUNCTION public.process_audit_log();

-- Function to create a new allocation
CREATE OR REPLACE FUNCTION public.create_po_unit_allocation(
    allocation_data JSONB
) RETURNS RECORD 
LANGUAGE plpgsql
AS $FUNCTION$
DECLARE
    new_allocation RECORD;
BEGIN
    INSERT INTO public.po_unit_allocations (po_recipe_id, po_raw_line_id, allocated_units, allocation_type)
    VALUES ( (allocation_data->>'po_recipe_id')::BIGINT, (allocation_data->>'po_raw_line_id')::BIGINT, (allocation_data->>'allocated_units')::INTEGER, allocation_data->>'allocation_type')
    RETURNING * INTO new_allocation;
    RETURN new_allocation;
END;
$FUNCTION$;

-- Function to bulk create allocations
CREATE OR REPLACE FUNCTION public.bulk_create_po_unit_allocations(
    allocations_array JSONB
) RETURNS SETOF JSONB
LANGUAGE plpgsql
AS $FUNCTION$
BEGIN
-- 1. Validation
    IF EXISTS (
        SELECT 1 FROM jsonb_array_elements(allocations_array) AS allocation_line WHERE jsonb_typeof(allocation_line) <> 'object'
    ) THEN
        RAISE EXCEPTION 'Each allocation entry must be an object.';
    END IF;

    -- 2. Bulk Insert and instantly output as JSON
    RETURN QUERY
    WITH inserted_rows AS (
        INSERT INTO public.po_unit_allocations (po_recipe_id, po_raw_line_id, allocated_units, allocation_type)
        SELECT 
            nullif(allocation_line->>'po_recipe_id', '')::BIGINT,
            nullif(allocation_line->>'po_raw_line_id', '')::BIGINT,
            nullif(allocation_line->>'allocated_units', '')::INTEGER,
            nullif(allocation_line->>'allocation_type', '')
        FROM jsonb_array_elements(allocations_array) AS allocation_line
        RETURNING *
    )
    SELECT to_jsonb(ir) FROM inserted_rows ir; 
    -- FUTURE PROOFING: If you want to stitch fields later, you just change line 24 to:
    -- SELECT to_jsonb(ir) || jsonb_build_object('case_name', c.name) FROM inserted_rows ir JOIN cases c ON ...
END;
$FUNCTION$;

-- Function to update an existing allocation
CREATE OR REPLACE FUNCTION public.update_po_unit_allocation(
    allocation_data JSONB
) RETURNS RECORD
LANGUAGE plpgsql
AS $FUNCTION$
DECLARE
    updated_allocation RECORD;
BEGIN
    UPDATE public.po_unit_allocations
    SET po_recipe_id = (allocation_data->>'po_recipe_id')::BIGINT,
        po_raw_line_id = (allocation_data->>'po_raw_line_id')::BIGINT,
        allocated_units = (allocation_data->>'allocated_units')::INTEGER,
        allocation_type = allocation_data->>'allocation_type'
    WHERE po_unit_allocation_id = (allocation_data->>'po_unit_allocation_id')::BIGINT
    RETURNING * INTO updated_allocation;
    RETURN updated_allocation;
END;
$FUNCTION$;

-- Function to bulk update allocations
CREATE OR REPLACE FUNCTION public.bulk_update_po_unit_allocations(
    allocations_array JSONB
) RETURNS SETOF JSONB
LANGUAGE plpgsql
AS $FUNCTION$
...
-- 1. Validation
    ...
    WITH updated_rows AS (
        UPDATE public.po_unit_allocations AS p
        SET 
            -- Update the editable planning fields
            po_recipe_id = updated.po_recipe_id,
            po_raw_line_id = updated.po_raw_line_id,
            allocated_units = updated.allocated_units,
            allocation_type = updated.allocation_type
        FROM (
            -- Unpack the incoming JSON array into a virtual database table
            SELECT 
                (allocation_line->>'po_unit_allocation_id')::BIGINT AS po_unit_allocation_id,
                nullif(allocation_line->>'po_recipe_id', '')::BIGINT AS po_recipe_id,
                nullif(allocation_line->>'po_raw_line_id', '')::BIGINT AS po_raw_line_id,
                nullif(allocation_line->>'allocated_units', '')::INTEGER AS allocated_units,
                nullif(allocation_line->>'allocation_type', '') AS allocation_type
            FROM jsonb_array_elements(allocations_array) AS allocation_line
        ) AS updated
        WHERE p.po_unit_allocation_id = updated.po_unit_allocation_id -- Match on your primary key to update the correct rows
        RETURNING p.* -- Capture the newly updated database state
    )
    SELECT to_jsonb(ur) FROM updated_rows ur; -- Transform rows into JSON on the fly
...
begin
  ...
  po_cases as (
    select
      ...
      p.is_processed as product_is_processed
    from cases c
  ...
  individual_boxes_agg as (
    ...
    from po_cases c
    where c.product_is_processed = false
  ...
  grouped_boxes as (
    ...
    from (
      ...
      from po_cases
      where product_is_processed = false
      group by purchase_order_id, product_product_id, product_name, product_item_num, product_upc, product_fnsku, product_asin, units_per_case, location_id, location_name
    ) pb
  ...
  grouped_cases as (
    ...
    from (
      select
        ...
        coalesce(sum(units_per_case), 0) as total_units
      from po_cases
      where product_is_processed = true
  ...
  po_raw_lines_with_products as (
    select
      ...
      COALESCE(
        (
          SELECT jsonb_agg(to_jsonb(alloc))
          FROM public.po_unit_allocations alloc
          WHERE alloc.po_raw_line_id = prl.po_raw_line_id
        ),
        '[]'::jsonb
      ) AS allocations_json
      ...
      coalesce(
        jsonb_agg(
          jsonb_build_object(
            ...
            'allocations', coalesce(prl.allocations_json, '[]'::jsonb)
