-- In the po details rpc, I need to stitch the allocated locations array into the po_raw_lines array for ease of access. 
...
begin
  ...
  po_raw_lines_with_products as (
    select
      ...
      coalesce(a.allocations_json, '[]'::jsonb) as raw_unit_allocations,
      coalesce(a.planned_fba_prep, 0) as planned_fba_prep,
      coalesce(a.received_fba_prep, 0) as received_fba_prep,
      coalesce(a.planned_fbm, 0) as planned_fbm,
      coalesce(a.received_fbm, 0) as received_fbm,
      coalesce(a.planned_store, 0) as planned_store,
      coalesce(a.received_store, 0) as received_store,
      coalesce(a.storage_locations, '[]'::jsonb) as storage_locations
    from po_raw_lines prl
    left join products p on p.product_id = prl.product_id

    left join lateral (
    select
        jsonb_agg(to_jsonb(alloc) order by alloc.po_unit_allocation_id) as allocations_json,
        sum(case when alloc.allocation_type = 'fba_prep' then alloc.allocated_units else 0 end) as planned_fba_prep,
        sum(case when alloc.allocation_type = 'fba_prep' then alloc.received_units else 0 end) as received_fba_prep,
        sum(case when alloc.allocation_type = 'fbm'      then alloc.allocated_units else 0 end) as planned_fbm,
        sum(case when alloc.allocation_type = 'fbm'      then alloc.received_units else 0 end) as received_fbm,
        sum(case when alloc.allocation_type = 'store'  then alloc.allocated_units else 0 end) as planned_store,
        sum(case when alloc.allocation_type = 'store'    then alloc.received_units else 0 end) as received_store,
        coalesce(
          jsonb_agg(alloc.storage_locations)
            filter (
              where alloc.allocation_type = 'store'
                and alloc.storage_locations is not null
            ),
          '[]'::jsonb
      ...
      coalesce(
        jsonb_agg(
          jsonb_build_object(
            ...
            'planned_fba_prep', prl.planned_fba_prep,
            'received_fba_prep', prl.received_fba_prep,
            'planned_fbm', prl.planned_fbm,
            'received_fbm', prl.received_fbm,
            'planned_store', prl.planned_store,
            'received_store', prl.received_store,
            'storage_locations', coalesce(prl.storage_locations, '[]'::jsonb),
            'allocations', coalesce(prl.raw_unit_allocations, '[]'::jsonb)
  ...
  all_products_agg as (
    ...
    where p.product_id in (select distinct product_id from po_cases)
       ...
       or p.product_id in (select distinct output_product_id from recipes_for_page)
  ),
  ...
  all_products_ids_agg as (
    ...
    from (
      ...
      union
      select output_product_id from recipes_for_page
    ) product_ids
  ),
